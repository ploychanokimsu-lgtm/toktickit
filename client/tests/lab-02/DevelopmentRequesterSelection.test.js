...
import { createTicket, getCategories, getDevelopmentRequesters, getRelatedSystems, } from "../../src/api";
vi.mock("../../src/api", () => ({
    checkSystem: vi.fn(),
...
describe("Development Requester Selection", () => {
    beforeEach(() => {
        ...
        mockedGetRelatedSystems.mockResolvedValue(relatedSystems);
    ...
    it("shows a loading state while Requesters are loading", () => {
        mockedGetDevelopmentRequesters.mockReturnValue(new Promise(() => { }));
        render(_jsx(App, {}));
    ...
    it("stores the selected Requester and opens the application", async () => {
        ...
        expect(sessionStorage.getItem("developmentRequesterId")).toBe("4");
        expect(await screen.findByRole("heading", {
            name: /create ticket/i,
        })).toBeInTheDocument();
        expect(screen.getAllByText("Ploy Srisuk")
    ...
    it("shows a safe API failure state and allows retry", async () => {
        mockedGetDevelopmentRequesters
            .mockRejectedValueOnce(new Error("Unable to load Development Requesters."))
        ...
        await waitFor(() => {
            expect(mockedGetDevelopmentRequesters).toHaveBeenCalledTimes(2);
        });
