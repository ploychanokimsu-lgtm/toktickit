...
describe("My Tickets", () => {
    ...
    it("shows the selected Requester's Tickets", async () => {
        ...
        await openMyTickets();
        expect(await screen.findAllByText("TKT-2026-000011")).not.toHaveLength(0);
    ...
    it("sends the selected Requester ID in the ownership header", async () => {
        ...
        await openMyTickets();
        await waitFor(() => {
            const call = fetchMock.mock.calls.find(([input]) => String(input).includes("/api/tickets?"));
    ...
    it("supports search, Category, Priority, and sorting", async () => {
        ...
        await selectRequester();
        await openMyTickets();
        await screen.findAllByText("TKT-2026-000011");
        ...
        await waitFor(() => {
            ...
            expect(urls.some((url) => url.includes("search=battery") &&
                url.includes("categoryId=1") &&
                url.includes("requestedPriority=HIGH") &&
                url.includes("sortOrder=asc"))).toBe(true);
        });
    });
    it("shows a no-results state for active search criteria", async () => {
        ...
        expect(await screen.findByText(/no tickets match your current search or filters/i)).toBeInTheDocument();
    });
