        ...
        if (url.endsWith("/api/tickets") &&
            ...
            return jsonResponse({
                ticket: {
                    ...
                    clientSubmissionId: "test-submission-id",
                    requesterId: 1,
                    categoryId: 2,
                    relatedSystemId: 1,
                    summary: "Laptop battery drains quickly",
                    requestedPriority: "MEDIUM",
                    description: "Laptop battery drains within one hour of normal use.",
                    currentStatus: "NEW",
...
async function fillValidTicketForm() {
    ...
    fireEvent.change(screen.getByLabelText(/ticket summary/i), {
        target: {
            value: "Laptop battery drains quickly",
        },
    });
    fireEvent.change(screen.getByLabelText(/^description/i), {
        target: {
            value: "Laptop battery drains within one hour of normal use.",
        },
    });
    ...
    expect(screen.getByLabelText(/^description/i)).toHaveValue("Laptop battery drains within one hour of normal use.");
}
describe("Create Ticket", () => {
    ...
    it("shows the Create Ticket screen after selecting a Requester", async () => {
    ...
    it("loads Category and Related System reference data", async () => {
        installSuccessfulApiMock();
        render(_jsx(App, {}));
        await selectRequester();
        await waitForTicketFormReady();
    ...
    it("uses MEDIUM as the default Requested Priority", async () => {
        installSuccessfulApiMock();
        render(_jsx(App, {}));
        await selectRequester();
        await waitForTicketFormReady();
    ...
    it("shows field-level validation and does not submit invalid data", async () => {
        const fetchMock = installSuccessfulApiMock();
        render(_jsx(App, {}));
        await selectRequester();
        await waitForTicketFormReady();
        ...
        expect(screen.getByText(/ticket summary must contain at least 5 characters/i)).toBeInTheDocument();
        expect(screen.getByText(/description must contain at least 10 characters/i)).toBeInTheDocument();
        const ticketCalls = fetchMock.mock.calls.filter(([input, init]) => String(input).endsWith("/api/tickets") &&
            init?.method === "POST");
        expect(ticketCalls).toHaveLength(0);
    });
    it("creates a valid Ticket and displays the official Ticket Number", async () => {
        const fetchMock = installSuccessfulApiMock();
        render(_jsx(App, {}));
        await selectRequester();
        ...
        expect(screen.getAllByText(/TKT-2026-000010/i).length).toBeGreaterThan(0);
    });
    it("preserves entered values when Ticket creation fails", async () => {
        const fetchMock = installFailingTicketApiMock();
        render(_jsx(App, {}));
        await selectRequester();
        await fillValidTicketForm();
        fireEvent.click(screen.getByRole("button", {
        ...
        expect(await screen.findByText(/ticket could not be created/i)).toBeInTheDocument();
        expect(screen.getByLabelText(/ticket summary/i)).toHaveValue("Laptop battery drains quickly");
        expect(screen.getByLabelText(/^description/i)).toHaveValue("Laptop battery drains within one hour of normal use.");
        expect(screen.getByLabelText(/^category/i)).toHaveValue("2");
        expect(screen.getByLabelText(/related system/i)).toHaveValue("1");
